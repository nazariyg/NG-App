...
public final class HomeScene: UIInitialScene {
    ...
    public init() {
        DispatchQueue.syncSafeOnMain {
            components.view =
                InstanceService.shared.instance(for: HomeViewProtocol.self, defaultInstance: HomeView())
            ...
            workerQueueScheduler.schedule { [components, _sceneIsInitialized, disposeBag] in
                components.interactor =
                    InstanceService.shared.instance(
                        for: HomeInteractorProtocol.self,
                        defaultInstance: HomeInteractor(workerQueueScheduler: workerQueueScheduler))
                components.presenter =
                    InstanceService.shared.instance(
                        for: HomePresenterProtocol.self,
                        defaultInstance: HomePresenter(workerQueueScheduler: workerQueueScheduler))
                components.router =
                    InstanceService.shared.instance(for: HomeRouterProtocol.self, defaultInstance: HomeRouter())
    ...
    struct WorkerQueueSchedulerWiring {
        ...
        struct ViewEvents {
            let events: O<HomeView.Event>
        }

        let interactorGo: O<Void>
        let viewOnMainScheduler: O<HomeViewProtocol>
        ...
        var interactor: (events: O<HomeInteractor.Event>, requests: O<HomeInteractor.Request>) {
            let events =
        ...
        var presenter: (events: O<HomePresenter.Event>, requests: O<HomePresenter.Request>) {
            let events =
        ...
        func biBind<BiBindableType: BiBindable>(
            _ variable: V<BiBindableType.Element>, toView viewPropertyKeyPath: KeyPath<HomeViewProtocol, BiBindableType>, disposeBag: DisposeBag) {
